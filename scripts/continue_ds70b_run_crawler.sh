...
CUDA_VISIBLE_DEVICES=3,4 nohup python exp/run_crawler.py \
    --device "cuda" \
    --cache_dir "/disk/u/models/" \
    --model_path "deepseek-ai/DeepSeek-R1-Distill-Llama-70B" \
    --quantization_bits 8 \
    --prompt_injection_location "thought_prefix" \
    --load_fname "crawler_log_20250328_012647_DeepSeek-R1-Distill-Llama-70B_1samples_100000crawls_Truefilter_thought_prefixprompt_q8.json" \
    "$@" \
    > "$LOG_FILE" 2>&1 &
