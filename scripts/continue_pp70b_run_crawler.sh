...
CUDA_VISIBLE_DEVICES=5,6 nohup python exp/run_crawler.py \
    --device "cuda" \
    --cache_dir "/disk/u/models/" \
    --model_path "perplexity-ai/r1-1776-distill-llama-70b" \
    --quantization_bits 8 \
    --prompt_injection_location "thought_prefix" \
    --load_fname "crawler_log_20250328_012541_r1-1776-distill-llama-70b_1samples_100000crawls_Truefilter_thought_prefixprompt_q8.json" \
    "$@" \
    > "$LOG_FILE" 2>&1 &
