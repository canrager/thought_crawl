...
from core.tokenization_utils import (
    get_special_tokens,
    custom_decoding,
    custom_batch_encoding,
)
...
def load_prompts(
    result_dir: Path, filename: str, num_prompts: int
...
def load_eval_tokens(
    model_name: str, tokenizer: AutoTokenizer, thinking_message: str = "None"
) -> List[str]:
    """
    ...
    batch_encodings = custom_batch_encoding(
        model_name, tokenizer, prompts, thinking_message=thinking_message
    )
...
def compute_mean_diff_vector(
    non_refusal_activations: torch.Tensor, refusal_activations: torch.Tensor
) -> torch.Tensor:
    """
